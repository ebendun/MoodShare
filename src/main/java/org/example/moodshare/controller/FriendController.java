...
public class FriendController {
    ...
    public ResponseEntity<?> getFriendRequests(@AuthenticationPrincipal UserDetails userDetails) {
        ...
        // 将请求列表转换为简单信息
        Set<Map<String, Object>> requestInfos = requests.stream().map(requester -> {
            Map<String, Object> requestInfo = new HashMap<>();
            requestInfo.put("id", requester.getId());
            requestInfo.put("username", requester.getUsername());
            requestInfo.put("profilePicture", requester.getProfilePicture());
            return requestInfo;
        }).collect(Collectors.toSet());
